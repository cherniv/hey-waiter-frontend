...
export class Calc {
    static log(base: number, val: number): number{
        return Math.log(val) / Math.log(base);
    }

    static mix(a: number, b: number, val: Value): number{
    ...
    static rmx(o: { from: number[], to: number[], val: number }): Value{
        return Calc.remix(o.from[0], o.from[1], o.val, o.to[0], o.to[1]);

    }

    static ro(v: number, factor: number){
        const a = Math.pow(10, factor);
